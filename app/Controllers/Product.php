...
class Product extends Controller
{
    ...
    public function index()
    {
        ...
        $data['category'] = $category; // membawa data ke halaman index
        $data['keyword'] = $keyword; // ------||-------

        //untuk mengelompokan hanya field name dan id pada table category
        ...
        if(!empty($keyword)){
        	$like = ['product.product_nmae' => $keyword];
        	$or_like = ['product.product_sku' => $keyword, 'product.product_description' => $keyword];
        }

        // end filter

    	// paginate
    	$paginate = 5; // jumlah perhalaman
...
public function delete($id)
{
	...
	if($hapus)
	{
		session()->setFlashdata('warning', 'Deleted Product seccesfully');
		return redirect()->to(base_url('product'));
	}
}
}
