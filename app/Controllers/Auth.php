...
class Auth extends BaseController
{
    ...
    {
        if($this->cek_login() == TRUE){ // untuk mengecek apabila user sudah login
            return redirect()->to('/dashboard'); //maka akan diarahkan ke halaman dashboard
        }
    ...
    public function proses_register()
    {
        ...
        if($validation->run($data, 'authregister') == FALSE){
            ...
            $datalagi = [
                ...
                'name'          => $this->request->getPost('name'),
                'username'      => $this->request->getPost('username'),
                'password'      => password_hash($this->request->getPost('password'), PASSWORD_DEFAULT), // untuk membuat generate password
                'status'        => "Active",
                'level'         => "Admin"
            ];
